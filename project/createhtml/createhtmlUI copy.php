...
<html>
  ...
  <body>
    ...
    <form action="../logout.php">
      <button type="submit" name="logout" value="ログアウト">
    </form>
    -->
    <!--メニュー：終了-->
    <!--コンテンツ：開始-->
...
if(!empty($results)){
    ...
    //$rowの中にはテーブルのカラム名が入る
    echo '<p><a href="editSection.php?contents_id='.$row['sectionId'].'&contentsRef='.$title.'; ?>">編集</a>
          <a href="deleteSection.php?message_id='.$row['sectionId'].'&contentsRef='.$title.'; ?>">削除</a></p>';
    echo $row['contents'];
  }
}
?>
      ...
      <?php
        ...
        if(isset($_GET['editor2'])){
          if(!empty($_GET['editor2'])){
            //パラメタの受け取り
            $documents =$_GET['editor2'];
            //ファイルを開く（追記モード）
            $fp = fopen($filename,"w");
            //ファイルへの書き込み（条件付き）
            fwrite($fp,$documents.PHP_EOL);
            //ファイルを閉じる
            fclose($fp);    
            print $_GET['editor2'];
          }else{
